...
uint256 hashGenesisBlock("0x77006c2d4fd3f1b8c9d63aad675da9e388b5c9776ef00eca4e298c07afdef825");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(537261531); // Joebarack: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Joebarack Signed Message:\n";
...
{
    // Joebarack: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 6000000000;

            if(nHeight == 1)
            {
            nSubsidy = 20000000000000;
			}
			
    // Subsidy is cut in half every 700800 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 700800); // Joebarack: 700.8k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 450; // Joebarack: 7.5 minutes
static const int64 nTargetSpacing = 90; // Joebarack: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Joebarack: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Joebarack: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Joebarack: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xedb41d24d87e160bc0e4ff331089b6c59caf4cf4b48c81dad6102f1224ad1777");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Joebarack JOBAR always wins";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 6000000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("09d3d520c4cae3f01f14928a4a3e3bd7083dfd3f2b4c4f84e18df4938b627e9bb81b58491188f8605fbbe108688c015f2b5df65ebc7de77a48cfcf79f9a7950d66") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1479177423;
        block.nBits    = 537261531;
        block.nNonce   = 1153604319;

        if (fTestNet)
        {
            block.nTime    = 1479177423;
            block.nNonce   = 4052236634;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xfebb8387691b513296f9259980a1ca559188f23d8e2e7254468f2b8b6a42e8d9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Joebarack: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JoebarackMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JoebarackMiner : ProcessBlock, block not accepted");
    }
